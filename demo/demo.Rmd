...
![The Palmer Archipelago penguins. Artwork by @allison_horst.](penguins.png)
...
The average weight of a penguin is `r cloze(round(mean(penguins$body_mass_g, na.rm = TRUE)), tolerance = 1)`, which is slightly `r cloze("more", c("more", "less"))` than the median weight of `r cloze(median(penguins$body_mass_g, na.rm = TRUE), tolerance = 1)`.
...
```{r boxplot, fig.cap="Boxplots of penguin weight by species", warning=FALSE}
library(ggplot2)
penguins |> 
...
```{r boxplot-sex, fig.cap="Boxplots of penguin weight by species and sex", warning=FALSE}
penguins |> 
